...
fn aes256gcm_encrypt_with_iv(
    ...
    let unbondkey = aead::UnboundKey::new(&aead::AES_256_GCM, &item.key)?;
    let lesssafekey = aead::LessSafeKey::new(unbondkey);
    lesssafekey.seal_in_place_append_tag(nonce, aead::Aad::empty(), &mut data)?;
    Ok(data)
}
...
fn aes256gcm_decrypt_with_iv(
    ...
    let unbondkey = aead::UnboundKey::new(&aead::AES_256_GCM, &item.key)?;
    let lesssafekey = aead::LessSafeKey::new(unbondkey);
    let plaintext = lesssafekey.open_in_place(nonce, aead::Aad::empty(), data)?;
    Ok(plaintext.to_vec())
}
...
fn aes256gcm_encrypt(item: &AES256GCMCodecItem, data: Vec<u8>) -> Result<Vec<u8>> {
    ...
    rng.fill(&mut randombytes)?;
    let raw_nonce = &randombytes[..];
    let ciphertext = aes256gcm_encrypt_with_iv(item, data, raw_nonce)?;
    let mut output = randombytes.to_vec();
...
fn aes256gcm_decrypt(item: &AES256GCMCodecItem, mut data: Vec<u8>) -> Result<Vec<u8>> {
    ...
    aes256gcm_decrypt_with_iv(item, ciphertext, raw_nonce)
}
...
impl ClientCoding for AES256GCMCodecItem {
    ...
    fn encode_data_with_iv(&mut self, input: Vec<u8>, iv: &[u8]) -> Result<Vec<u8>> {
        self.counter += 1;
        aes256gcm_encrypt_with_iv(self, input, iv)
    }

    fn decode_data_with_iv(&mut self, mut input: Vec<u8>, iv: &[u8]) -> Result<Vec<u8>> {
        aes256gcm_decrypt_with_iv(self, &mut input[..], iv)
    }
}
